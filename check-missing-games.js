...
// 检查每个游戏是否有对应的文件
const missingGames = GameLibrary.filter(game => {
    if (!game.urlPath) return false; // 跳过没有urlPath的游戏
    const expectedFile = `${game.urlPath}.html`;
    return !existingFiles.includes(expectedFile);
});

console.log('Missing game files:');
missingGames.forEach(game => {
    console.log(`ID: ${game.id} - ${game.name} (${game.urlPath}.html)`);
});

// 生成ID列表
const missingIds = missingGames.map(game => game.id).sort((a, b) => a - b);
console.log('\nMissing game IDs in array format:');
console.log(`[${missingIds.join(', ')}]`);

console.log(`\nTotal missing: ${missingGames.length}`);
console.log(`Total games: ${GameLibrary.length}`);
